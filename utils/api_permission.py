...
async def check_api_permissions(
    ...
    permissions_map = API_PERMISSIONS[api_name]
   
    if feature:
        if isinstance(permissions_map, dict):
            required_permissions = permissions_map.get(feature)
            if required_permissions is None:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"No permissions defined for API '{api_name}' with feature '{feature}'"
                )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"API '{api_name}' does not support feature-specific permissions"
            )
    else:
        if isinstance(permissions_map, dict):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"API '{api_name}' requires a feature to determine permissions"
            )
        required_permissions = permissions_map
    ...
    if missing_permissions:
        raise HTTPException(
            ...
            detail=f"Access denied - You don't have permission for: {api_name} (feature: {feature or 'N/A'})"
        )